...
public class AvailabilityProviderTwoService: KafkaEdaBase, IKafkaEvent
{
    protected readonly IMediator _mediator;
    protected readonly IMapper _mapper;

    public AvailabilityProviderTwoService(IKafkaConfiguration configuration, IMediator mediator, IMapper mapper)
        : base(configuration)
    {
        this._mapper = mapper;
    ...
    public override Task Handler(ConsumeResult<Ignore, string> result)
        => Task.Factory.StartNew(() =>
        {
            ...
            _mediator.Send(_mapper.Map<AvailabilityTwoRequest>(request));
        });
}
